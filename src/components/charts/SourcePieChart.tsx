...
export default function SourcePieChart({ data }: Props) {
  const labeled = data.map((d) => ({ ...d, site: SOURCE_LABEL[d.site] ?? d.site }));
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5">
      ...
      <ResponsiveContainer width="100%" height={220}>
        <PieChart>
          <Pie data={labeled} dataKey="count" nameKey="site" cx="50%" cy="50%" outerRadius={80} label={({ site, percent }) => `${site} ${(percent * 100).toFixed(0)}%`} labelLine={false}>
            {labeled.map((_, i) => (
